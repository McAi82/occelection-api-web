...
const MonitoringDashboard: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const electionChannel = subscribeToElectionChannel(
            ...
            (data) => {
                if (data && data.turnout !== undefined) {
    ...
    useEffect(() => {
        if (selectedElection && monitoringData?.is_ongoing) {
            const interval = setInterval(() => {
                refreshData();
            }, 60000);
            return () => clearInterval(interval);
        }
    }, [selectedElection, monitoringData?.is_ongoing]);
    ...
    const fetchMonitoringData = async () => {
        ...
        try {
            const response =
                await monitoringAPI.getDashboard(selectedElectionId);
    ...
    const handleRealtimeUpdate = (data: any) => {
        if (data && data.turnout !== undefined) {
            ...
            const newPoint: TurnoutHistoryPoint = {
                ...
                turnout: data.percentage || data.turnout || 0,
                votes: data.voted_count || 0,
                timestamp: now,
            };
            ...
            if (monitoringData) {
                setMonitoringData({
                    ...
                    statistics: {
                        ...
                        voted_count:
                            data.voted_count ||
                            monitoringData.statistics.voted_count,
                        turnout_percentage:
                            data.percentage ||
                            data.turnout ||
                            monitoringData.statistics.turnout_percentage,
                    },
                });
            }
            setHasRealData(true);
    ...
    const getFilteredHistory = (): TurnoutHistoryPoint[] => {
        ...
        switch (selectedTimeRange) {
            ...
            case "1hour":
                ...
                break;
            case "3hours":
                cutoff = now - 180 * 60000;
                break;
            default:
                cutoff = now - 120 * 60000;
                break;
        }
    ...
    const preparePositionProgressData = (): Array<{
        name: string;
        votes: number;
        candidates: number;
        progress: number;
    ...
    const latestTurnout =
        filteredHistory.length > 0
            ? filteredHistory[filteredHistory.length - 1].turnout
            : 0;
    ...
    return (
        <div className="space-y-6">
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                        <div>
                            ...
                            <div className="flex flex-wrap gap-4 mt-2 text-sm text-gray-500">
                                ...
                                {selectedElectionData.course && (
                                    <div className="flex items-center gap-1">
                                        <Users className="w-4 h-4" />
                                        <span>
                                            {typeof selectedElectionData.course ===
                                                "object" &&
                                            "course_code" in
                                                selectedElectionData.course
                                                ? selectedElectionData.course
                    ...
                    <div className="h-[250px] flex items-center justify-center text-gray-500">
                        <div className="text-center">
                            ...
                            <p className="text-xs text-gray-400 mt-1">
                                Real-time data will appear once votes are cast
                            </p>
                        </div>
                    </div>
                )}
            ...
            {monitoringData?.position_progress &&
                monitoringData.position_progress.length > 0 && (
                    <div className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm">
                        <div className="flex items-center gap-2 mb-4">
                            <Target className="w-5 h-5 text-blue-600" />
                            <span className="text-sm font-semibold text-gray-900">
                            ...
                            <Badge className="bg-blue-100 text-blue-700 text-[10px] border-0 ml-auto">
                                {monitoringData.position_progress.length}{" "}
                                positions
                            </Badge>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            {monitoringData.position_progress.map(
                                (position, idx) => (
                                    <div
                                        key={position.position_id}
                                        className={`p-4 rounded-xl border ${idx % 2 === 0 ? "border-blue-100 bg-blue-50/30" : "border-purple-100 bg-purple-50/30"}`}
                                    >
                                        <div className="flex justify-between items-center mb-2">
                ...
                <div className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm">
                    ...
                    <div className="flex items-center justify-between">
                        <span className="text-gray-700">
                            Votes in last 30 minutes
                        </span>
                        <span className="font-bold text-green-600 text-2xl">
                            {monitoringData?.recent_activity?.last_30_minutes ||
                                0}
                        </span>
                    </div>
                </div>
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        {isConnected ? (
                            ...
                            <Badge className="bg-yellow-100 text-yellow-700 border-0">
                                <span className="inline-block w-1.5 h-1.5 bg-yellow-500 rounded-full mr-1 animate-pulse"></span>
                                Reconnecting...
                            </Badge>
                        )}
                    </div>
